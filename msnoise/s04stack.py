                ...
                if len(updated_days) != 0:
                    ...
                    if nstack > 0:
                        if stype == "mov":
                                ...
                                for i, date in enumerate(datelists[mov_stack]):
                                    ...
                                    if newdata:
                                        ...
                                        if not np.all(np.isnan(corr)):
                                            ...
                                            if sac:
                                                export_sac(
                                                    station1, station2, db, filename, pair, components, filterid, corr, maxlag=maxlag, cc_sampling_rate=cc_sampling_rate) #add station1, station2
                                            day_name = "%s:%s" % (
                                                sta1, sta2)
                            ...
                            if sac:
                                export_sac(
                                    station1, station2, db, filename, pair, components, filterid, stack_total)   #add station1, station2
