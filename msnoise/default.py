...
default["response_path"] = ["Instrument correction file(s) location (path relative to db.ini), "
                            "defaults to './inventory', i.e. a subfolder in the current project folder."
                            "<br>All files in that folder will be parsed.", "inventory"]
default["response_prefilt"] = ["Remove instrument correction **pre-filter** (0.005, 0.006, 30.0, 35.0)",
...
default["windsorizing"] = ["Windsorizing at N time RMS , 0 disables windsorizing, -1 enables 1-bit normalization [3]",
                           "3"]
default["whitening"] = ["Whiten Traces before cross-correlation: All (except for autocorr), None, "
                        "or only if components are different [A]/N/C", "A"]
...
default["mov_stack"] = ["Number of days to stack for the Moving-window stacks ([5]= [day-4:day]), "
                        "can be a comma-separated list 1,2,5,10", "5"]
...
default["dtt_mincoh"] = ["Minimum coherence on dt measurement, "
                         "MWCS points with values lower than that will **not** be used in the WLS", "0.65"]
default["dtt_maxerr"] = ["Maximum error on dt measurement, "
                         "MWCS points with values larger than that will **not** be used in the WLS", "0.1"]
default["dtt_maxdt"] = ["Maximum dt values, MWCS points with values larger than that will **not** be used in the WLS",
                        "0.1"]
