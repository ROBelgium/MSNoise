...
def main(loglevel="INFO"):
    ...
    logger.info('*** Starting: Wavelet DTT ***')
    ...
    while True:
        ...
        days = batch["days"]
        params = batch["params"]
        # lineage_names[:-1] ends with the upstream "wavelet" step name —
        # used for both xr_load_wct (read) and xr_save_wct_dtt2 (write prefix)
        ...
        logger.info(
            f"New Wavelet DTT Job: pair={pair} n_days={len(days)} lineage={lineage_str}"
        )
        ...
        dtt_freqmin = float(params.wct_dtt_freqmin)
        dtt_freqmax = float(params.wct_dtt_freqmax)
        minlag = float(params.wct_minlag)
        lag_type = str(params.wct_lag)
        width = float(params.wct_width)
        v = float(params.wct_v)
        sides = str(params.wct_sides)
        mincoh = float(params.wct_mincoh)
        maxdt = float(params.wct_maxdt)
        coda_cycles = int(params.wct_codacycles)
        min_nonzero = float(params.wct_min_nonzero)
        ...
        for component in components_to_compute:
            for mov_stack in mov_stacks:
                ...
                try:
                    ...
                    logger.info(
                        f"Saved wavelet_dtt for {pair}/{component}/{mov_stack} "
                        f"({len(dates_out)} days)"
                    )
                except Exception as e:
                    logger.error(
                        f"Error saving wavelet_dtt for {pair}/{component}/{mov_stack}: {e}"
                    )
    ...
    logger.info('*** Finished: Wavelet DTT ***')
