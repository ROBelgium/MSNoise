...
def eruption(begin, end, label=False):
    if label:
        plt.axvspan(begin, end, zorder=-100,
                    color="red", alpha=0.5, label='Eruptions')
    else:
        plt.axvspan(begin, end, zorder=-100, color="red", alpha=0.5)


def eruptions():
    eruption(datetime.date(2009, 11, 5),
             datetime.date(2009, 11, 6), label=True)
